...
template<bool A_in_regs=false, bool B_in_regs=false, bool C_in_regs=false, 
         typename Tensor0, typename Tensor1, typename Tensor2, typename Tensor3, 
         typename Tensor4, typename Tensor5, typename Tensor6, typename TiledMma, 
...
__forceinline__ __device__ void gemm_kvclus(
    ...
    ThrCopyA smem_thr_copy_A, ThrCopyB smem_thr_copy_B,
    ThrCopyC smem_thr_copy_C) {

    CUTE_STATIC_ASSERT_V(size<1>(tCrA) == size<1>(acc));                     // MMA_M
    CUTE_STATIC_ASSERT_V(size<1>(tCrB) == size<2>(acc));                     // MMA_N
    CUTE_STATIC_ASSERT_V(size<1>(tCrC) == size<1>(acc));                     // MMA_M
    CUTE_STATIC_ASSERT_V(size<2>(tCrC) == size<2>(acc));                     // MMA_N
    CUTE_STATIC_ASSERT_V(size<2>(tCrA) == size<2>(tCrB));                    // MMA_K
