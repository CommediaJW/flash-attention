...
void set_kvclus_params_fprop(
    Flash_kvclus_fwd_params &params,
    // sizes
    const size_t b,
    const size_t seqlen_q,
    const size_t seqlen_k,
...
void run_mha_fwd_kvclus(Flash_kvclus_fwd_params &params, cudaStream_t stream, bool force_split_kernel=false) {
    FP16_SWITCH(!params.is_bf16, [&] {
        HEADDIM_SWITCH(params.d, [&] {
            BOOL_SWITCH(params.is_causal, Is_causal, [&] {
                // TODO: run_mha_kvclus_fwd_splitkv_dispatch
                // if (params.num_splits <= 1 && !force_split_kernel) {  // If we don't set it num_splits == 0
                //     run_mha_fwd_<elem_type, kHeadDim, Is_causal>(params, stream);
                // } else {
                //     run_mha_fwd_splitkv_dispatch<elem_type, kHeadDim, Is_causal>(params, stream);
...
mha_fwd_kvclus(
    ...
    const at::Tensor &k,                        // batch_size x seqlen_k x num_heads_k x head_size
    const at::Tensor &v,                        // batch_size x seqlen_k x num_heads_k x head_size
    const at::Tensor &bias,                     // batch_size x seqlen_q x num_heads_k x seqlen_k
    c10::optional<at::Tensor> &out_,            // batch_size x seqlen_q x num_heads x head_size
    c10::optional<at::Tensor> &alibi_slopes_,   // num_heads or batch_size x num_heads
    const float p_dropout,
    const float softmax_scale,
    ...
    set_kvclus_params_fprop(
        ...
        num_heads, num_heads_k,
        head_size, head_size_rounded,
        q_padded, k_padded, v_padded, bias_padded, out,
        /*cu_seqlens_q_d=*/nullptr,
        /*cu_seqlens_k_d=*/nullptr,
        /*seqused_k=*/nullptr,
        return_softmax ? p.data_ptr() : nullptr,
    ...
    if (seqlen_k > 0) {
        auto stream = at::cuda::getCurrentCUDAStream().stream();
        run_mha_fwd_kvclus(params, stream);
    } else {
        // If seqlen_k == 0, then we have an empty tensor. We need to set the output to 0.
        out.zero_();
        softmax_lse.fill_(std::numeric_limits<float>::infinity());
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("fwd_kvclus", &mha_fwd_kvclus, "Forward pass of KVClus");
}
